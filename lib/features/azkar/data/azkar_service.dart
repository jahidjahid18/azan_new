...
class AzkarService {
  ...
  Future<Map<String, List<AzkarItem>>> loadAzkarByCategory() async {
    if (_cache != null) {
      return _cache!;
    }
    ...
    _cache = categories.map<String, List<AzkarItem>>((key, value) {
      final items = (value as List<dynamic>)
          .map((item) => AzkarItem.fromMap(item as Map<String, dynamic>))
          .toList(growable: false);
      return MapEntry<String, List<AzkarItem>>(key, items);
    });
